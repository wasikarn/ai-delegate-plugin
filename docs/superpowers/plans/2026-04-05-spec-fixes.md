# Spec Fixes: Generic Orchestration Design

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Update `docs/superpowers/specs/2026-04-05-generic-orchestration-design.md` to fix all critical/high issues identified by 3-agent review (architecture, security, code quality).

**Architecture:** Pure spec document rewrite — no new code in this plan. Each task rewrites specific sections of the spec and commits. The spec is the single source of truth for the subsequent Phase 1-3 implementation.

**Tech Stack:** Markdown, YAML frontmatter conventions for agent metadata.
...
| File | Change |
|------|--------|
| `docs/superpowers/specs/2026-04-05-generic-orchestration-design.md` | Rewrite 6 sections to fix all review findings |

No code files change in this plan. These are spec-only fixes.

---

## Background: What the Agents Found

Three review agents identified these clusters of issues:

**Critical:**

- Command injection in `ollama launch claude --agent <name>` (no validation)
- No trust boundary — any plugin's agents run with full tool access
- Path traversal in `catalog.scan()` via symlinks
- `ollama launch claude` flags unverified (may not exist)
- `catalog.py` API contract missing (domain matching algorithm undefined) → Phase 2 blocker

**High:**

- Orchestrator does 5 jobs in one Claude Code agent (SRP violation)
- Path selection algorithm not defined (what replaces SmartRouter?)
- Phase 3 test rewrite scope underestimated (~170 tests affected, no plan)
- Consensus pre-debate vs post-debate unclear
- Memory cache integration spec incomplete
- 4 execution paths over-engineered (drop Launch path B, Gemini path D)

---

## Task 1: Security Requirements + Simplify Execution Paths

**Files:**

- Modify: `docs/superpowers/specs/2026-04-05-generic-orchestration-design.md`

**What to change:**

1. Add new **Security Requirements** section after Architecture Overview
2. Replace "Expert Execution Paths" to remove Path B (ollama launch) and Path D (Gemini) — keep only Path A (SDK) and Path C (Claude Code Agent)
3. Update the architecture diagram to remove "ollama launch claude" line

- [ ] **Step 1: Add Security Requirements section**

Open the spec file. Find the line:
...
Insert this new section **before** that heading:
...
Catalog scan enforces all three:
...
- [ ] **Step 2: Simplify Execution Paths — keep A and C only**

Find and replace the entire "## Expert Execution Paths" section with this:

```markdown
## Expert Execution Paths

Two paths only. Path selection is determined by `ModelAssigner` (see Python Services section).

### Path A: Anthropic SDK (fast, pattern analysis)

Use for: secret detection, keyword scanning, text analysis where content is passed directly as a string.
...
**Model:** GLM-5 / Kimi via Ollama (~$1-5/M tokens)

**Limitation:** No file system access. Content must be passed as a string. If the task requires exploring a repo directory, use Path C instead.

### Path C: Claude Code Agent (deep reasoning + file access)

Use for: architecture review, complex multi-file security analysis, anything requiring file exploration or multi-step reasoning.
...
### Path B: Future (ollama launch claude) — NOT YET IMPLEMENTED

`ollama launch claude --agent X` is proposed as a "budget file-access path" (real tools via Kimi instead of Sonnet). **Do not implement in Phase 1 or Phase 2.** This path requires:

- Verifying `ollama launch claude` CLI interface and supported flags
- Confirming `--agent`, `--allowedTools`, `--output-format` flags exist in current Ollama version
- Security review of auth token handling

Implement Path B only after Path A and Path C are proven in production.
...
Find the architecture diagram in "## Architecture Overview". Replace the expert execution lines:
...
git add docs/superpowers/specs/2026-04-05-generic-orchestration-design.md
git commit -m "docs: add security requirements + simplify to 2 execution paths"
```

---

## Task 2: Complete catalog.py API Contract

**Files:**

- Modify: `docs/superpowers/specs/2026-04-05-generic-orchestration-design.md`

**What to change:** Replace the "## Agent Catalog" section with a complete spec including: frontmatter schema (matching existing agents), domain matching algorithm, error handling, caching strategy, and plugin source name extraction.

- [ ] **Step 1: Replace Agent Catalog section**

Find and replace the entire "## Agent Catalog" section with:
...
```bash
git add docs/superpowers/specs/2026-04-05-generic-orchestration-design.md
git commit -m "docs: complete catalog.py API contract with domain matching + security"
```

---

## Task 3: Split Orchestrator into Python Services + Path Selection Algorithm
...
1. Rewrite "## Intelligence Layer — Orchestrator Agent" to introduce Python microservices
2. Add path selection algorithm (replacing SmartRouter logic)
3. Add progressive escalation state machine

- [ ] **Step 1: Rewrite Intelligence Layer section**

Find and replace the entire "## Intelligence Layer — Orchestrator Agent" section (from `### File: agents/orchestrator.md` through the end of Step 5) with:
...
```bash
git add docs/superpowers/specs/2026-04-05-generic-orchestration-design.md
git commit -m "docs: split orchestrator into Python services + define path selection algorithm"
```

---

## Task 4: Consensus Clarity + FindingsCache + Error Handling
...
1. Replace "## Consensus & Verdict" section to clarify pre/post-debate
2. Add FindingsCache definition
3. Add Error Handling Strategy section

- [ ] **Step 1: Replace Consensus & Verdict section**

Find and replace the entire "## Consensus & Verdict" section with:
...
**Sparse topology note:** If `sparse_topology_k` is set (defer to Phase 2), experts see only k peers during debate. Post-debate findings differ from pre-debate findings. Pre-debate consensus is still the correct input to tier selection. Adjudicator output is always the final verdict regardless.
...
```bash
git add docs/superpowers/specs/2026-04-05-generic-orchestration-design.md
git commit -m "docs: clarify consensus pre/post-debate + add FindingsCache + error handling"
```

---

## Task 5: Update Implementation Phases (Test Migration + Sparse Topology Deferral)
...
1. Replace "## Python Utilities — What Changes" to add new files from Task 3
2. Replace "## Implementation Phases" with updated phases including test migration plan and sparse topology deferral

- [ ] **Step 1: Replace Python Utilities section**

Find and replace "## Python Utilities — What Changes" with:
...
- [ ] **Step 2: Replace Implementation Phases section**

Find and replace "## Implementation Phases" with:
...
- [ ] **Step 3: Update Findings Contract section**

The existing agents use `{"domain": "security", "findings": [...], "score": N}` format (not the spec's v1.0 contract). Add a migration note to the Findings Contract section.

Find the "## Findings Contract (JSON Schema v1.0)" section. Add this note after the schema block:
...
```bash
git add docs/superpowers/specs/2026-04-05-generic-orchestration-design.md
git commit -m "docs: update implementation phases with test migration plan + sparse topology deferral"
```
...
After all 5 tasks are done, verify:

- [ ] Security: S1 (agent name validation), S2 (trust boundary), S3 (path traversal), S4 (DoS) all have code examples
- [ ] Execution paths: Only A and C documented as active; B marked as "future/not yet"
- [ ] `catalog.py`: frontmatter schema matches existing `agents/*.md` format (name, description, model, color, tools)
- [ ] Domain matching algorithm: specific keywords + code example
- [ ] Path selection: clear decision tree with code (replaces SmartRouter)
- [ ] Consensus: pre-debate = tier selection, post-debate = adjudicator output
- [ ] `FindingsCache`: `CacheKey`, `CacheEntry`, `get`/`set` with TTL
- [ ] Phase 3 test migration: table with 170 affected tests + per-file action
- [ ] `sparse_topology_k`: "defer, keep field, don't expand" — not removed
- [ ] Existing agent output format: compatibility note added to Findings Contract
...
| Budget model routing via Path A (Kimi instead of Sonnet) | ~80% |
| Early exit on consensus | ~30-50% |
| Cache-first (skip re-analysis) | ~100% on repeat |
| Path A vs Path C routing for pattern analysis | ~70% |
| **Combined target** | **70-95% vs naive "spawn all Claude agents"** |
